defmodule AshJido.GeneratorTest do
  ...
  use ExUnit.Case, async: false  # Code generation touches global state
  ...
  describe "action type coverage" do
    test "update action schema includes id field" do
      ...
      module_name = AshJido.Generator.generate_jido_action_module(TestResource, jido_action, dsl_state)
    ...
    test "destroy action schema includes id field" do
      ...
      module_name = AshJido.Generator.generate_jido_action_module(TestResourceWithDestroy, jido_action, dsl_state)
  ...
  describe "default action naming" do
    test "uses default name when jido_action.name is nil" do
      ...
      jido_action = %AshJido.Resource.JidoAction{
        action: :register,
        name: nil,  # This will trigger default naming
        module_name: nil,
  ...
  describe "error handling" do
    test "determine_domain raises helpful error when no domain in context" do
      ...
      assert_raise ArgumentError, ~r/No domain found for resource/, fn ->
        module_name.run(%{}, %{})  # context without :domain
      end
    end
    ...
    test "destroy action requires an id parameter" do
      ...
      module_name = AshJido.Generator.generate_jido_action_module(TestResourceWithDestroy2, jido_action, dsl_state)
